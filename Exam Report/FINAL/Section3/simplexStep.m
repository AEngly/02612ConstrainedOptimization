function [x,fval,exitflag,output,lambda,Abar,bbar,gbar] = simplexStep(gbar,Abar,bbar,information,options)
    ...
    while ~converged && iter < maxiter
        ...
        if all(lambdaN >= -sqrt(eps))
            ...
            if isnan(J)
                ...
                x(iJ) = 0;
                x(iS) = alpha;
                iterates = [iterates x(1:nOriginal)];
                basic(j) = iS;
                nonBasic(s) = iJ;
                if verbose > 0
