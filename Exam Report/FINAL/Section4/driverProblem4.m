...
xk = ones(20,1);
[c,ceq,GC,GCeq] = conNLPTest(xk,20);
[f, Grad] = objNLPTest(xk,20);
...
[primal_final_1, dual_final_1, solverInformation_1] = SQPLineSearchBFGS(fun,x01,xlb,xub,clb,cub,nonlcon,options);
[primal_final_2, dual_final_2, solverInformation_2] = SQPLineSearchBFGS(fun,x02,xlb,xub,clb,cub,nonlcon,options);
[primal_final_3, dual_final_3, solverInformation_3] = SQPLineSearchBFGS(fun,x03,xlb,xub,clb,cub,nonlcon,options);
[primal_final_4, dual_final_4, solverInformation_4] = SQPLineSearchBFGS(fun,x04,xlb,xub,clb,cub,nonlcon,options);
...
legend([h5, h11, h13, h15, h14, h16, h18, h20],{'Local Minimum', 'Saddle Point', 'Local Maximum', 'Paths', sprintf('Solution with x_{0} = (%0.2f, %0.2f)',x01(1), x01(2)), sprintf('Solution with x_{0} = (%0.2f, %0.2f)',x02(1), x02(2)), sprintf('Solution with x_{0} = (%0.2f, %0.2f)',x03(1),x03(2)), sprintf('Solution with x_{0} = (%0.2f, %0.2f)',x04(1),x04(2))}, 'Location','southeast')
...
saveas(gcf,'./Plots/471_SQPLineSearchBFGSInfeasibilityHimmelblau.png')

close;

% Construct a table
columnNames = ["n", "x_{1}", "x_{2}"];
nIter = size(solverInformation_1.primalSequence',1);
TABLE = table((0:nIter-1)',solverInformation_1.primalSequence(1,:)',solverInformation_1.primalSequence(2,:)','VariableNames',columnNames);
table2latex(TABLE, './Data/47_P1.tex')

% Save as table
columnNames = ["n", "x_{1}", "x_{2}"];
nIter = size(solverInformation_2.primalSequence',1);
TABLE = table((0:nIter-1)',solverInformation_2.primalSequence(1,:)',solverInformation_2.primalSequence(2,:)','VariableNames',columnNames);
table2latex(TABLE, './Data/47_P2.tex')

% Save as table
columnNames = ["n", "x_{1}", "x_{2}"];
nIter = size(solverInformation_3.primalSequence',1);
TABLE = table((0:nIter-1)',solverInformation_3.primalSequence(1,:)',solverInformation_3.primalSequence(2,:)','VariableNames',columnNames);
table2latex(TABLE, './Data/47_P3.tex')

% Save as table
columnNames = ["n", "x_{1}", "x_{2}"];
nIter = size(solverInformation_4.primalSequence',1);
TABLE = table((0:nIter-1)',solverInformation_4.primalSequence(1,:)',solverInformation_4.primalSequence(2,:)','VariableNames',columnNames);
table2latex(TABLE, './Data/47_P4.tex')
...
[primal_final_1, dual_final_1, solverInformation_1] = SQPTrustRegion(fun,x01,xlb,xub,clb,cub,nonlcon,options);
[primal_final_2, dual_final_2, solverInformation_2] = SQPTrustRegion(fun,x02,xlb,xub,clb,cub,nonlcon,options);
[primal_final_3, dual_final_3, solverInformation_3] = SQPTrustRegion(fun,x03,xlb,xub,clb,cub,nonlcon,options);
[primal_final_4, dual_final_4, solverInformation_4] = SQPTrustRegion(fun,x04,xlb,xub,clb,cub,nonlcon,options);
...
saveas(gcf,'./Plots/481_SQPTrustRegionHimmelblau.png')
close;
...
nIter = size(solverInformation_1.primalSequence',1);
TABLE = table((0:nIter-1)',solverInformation_1.primalSequence(1,:)',solverInformation_1.primalSequence(2,:)','VariableNames',columnNames);
table2latex(TABLE, './Data/48_P1.tex')

% Save as table
columnNames = ["n", "x_{1}", "x_{2}"];
nIter = size(solverInformation_2.primalSequence',1);
TABLE = table((0:nIter-1)',solverInformation_2.primalSequence(1,:)',solverInformation_2.primalSequence(2,:)','VariableNames',columnNames);
table2latex(TABLE, './Data/48_P2.tex')

% Save as table
columnNames = ["n", "x_{1}", "x_{2}"];
nIter = size(solverInformation_3.primalSequence',1);
TABLE = table((0:nIter-1)',solverInformation_3.primalSequence(1,:)',solverInformation_3.primalSequence(2,:)','VariableNames',columnNames);
table2latex(TABLE, './Data/48_P3.tex')

% Save as table
columnNames = ["n", "x_{1}", "x_{2}"];
nIter = size(solverInformation_4.primalSequence',1);
TABLE = table((0:nIter-1)',solverInformation_4.primalSequence(1,:)',solverInformation_4.primalSequence(2,:)','VariableNames',columnNames);
table2latex(TABLE, './Data/48_P4.tex')

%% 4.9) Using test problem to compare performance to fmincon

% Use big constant for boundaries
M = 1e6;
n = 10;
x0 = ones(n,1)*0.50;
...
cub = [];
clb = [];
nonlcon = @(x) conNLPTest(x,n);
fun = @(x) objNLPTest(x,n);
...
% Solve with fmincon
optimoptions('fmincon','Display', 'iter', 'SpecifyObjectiveGradient', true, 'TolFun', 1e-8, 'Algorithm', 'interior-point');
[x_fmincon,fval,exitflag,output_fmincon,lambda,grad,hessian] = fmincon(fun, x0, A, b, Aeq, beq, lb, ub, nonlcon);

% Then we try with our solves
options = struct();
options.maxit = 1000;
options.BFGS = true;
options.stepSolver = "quadprog";
...
[primal_final_1, dual_final_1, solverInformation_1] = SQPTrustRegion(fun,x0,lb,ub,[],[],nonlcon,options);
